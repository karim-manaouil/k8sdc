...
def prepare(**kwargs):
    ...
    in_kubespray("cd %s && ansible-playbook -f 101 -i inventory/mycluster/hosts.ini"
                 " "
...
def post_install(**kwargs):
    ...
    run_ansible([os.path.join(ANSIBLE_DIR, "prepare_env_all.yml")],
                env["inventory"], extra_vars=extra_vars)

    run_ansible([os.path.join(ANSIBLE_DIR, "prepare_env.yml")],
                env["inventory"], extra_vars=extra_vars)
